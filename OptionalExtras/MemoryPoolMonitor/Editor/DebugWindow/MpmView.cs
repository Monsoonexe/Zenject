...
namespace Zenject.MemoryPoolMonitor
{
    public class MpmView : IGuiRenderable, ITickable, IInitializable
    {
        ...
        private bool ShouldIncludePool(IMemoryPool pool)
        {
            ...
            return _actualFilter.IsEmpty() || GetName(pool).ToLowerInvariant().Contains(_actualFilter);
        }
        ...
        private void DrawRowBackgrounds()
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                Texture2D background = pool.GetType() == _selectedPoolType
                    ? RowBackgroundSelected
                    : rowRect.Contains(mousePositionInContent) ? RowBackgroundHighlighted : i % 2 == 0 ? RowBackground1 : RowBackground2;
        ...
        {
            return index == 0 ? TotalWidth - ((NumColumns - 1) * _settings.NormalColumnWidth) : _settings.NormalColumnWidth;
        }
